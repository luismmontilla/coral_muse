...
```{r figure03a}
p3a <- ggplot(DF, aes(x = transects, y = mse)) +
...
```{r figure03b}
p3b <- plot_coefs(bc_mod$trafo_mod, colors = 'black', scale = TRUE) +
...
```{r}
data.frame(bc_mod$trafo_mod["coefficients"]) %>% 
  rownames_to_column('name') %>% 
  ggplot(aes(x = name, y = coefficients)) +
  geom_point() +
  coord_flip()
...
#ggsave('supplementary/supp01.svg')
```
