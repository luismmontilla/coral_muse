...
```{r}
ggplot(coral_centroids, aes(x = PCoA1, y = PCoA2, label = Site)) +
  geom_point() +
  geom_label_repel(aes(label = Site), point.padding = 0.5) +
  ylim(-1, 1) +
  xlim(-1, 1) +
  ggtitle('15q, 4t') +
  stat_ellipse(data = subset(coral_vectors, Site %in% c('Salinas')), alpha = 0.5) +
  stat_ellipse(data = subset(coral_vectors, Site %in% c('Charagato')), alpha = 0.5) +
  stat_ellipse(data = subset(coral_vectors, Site %in% c('Dos Mosquises Sur')), alpha = 0.5) +
  stat_ellipse(data = subset(coral_vectors, Site %in% c('Punta Conejo')), alpha = 0.5)
```

```{r}
ggplot(boot_centroids, aes(x = PCoA1, y = -PCoA2, label = Site)) +
  geom_point() +
  geom_label_repel(aes(label = Site), point.padding = 0.5) +
  ylim(-1, 1) +
  xlim(-1, 1) +
  ggtitle('10q, 10t') +
  stat_ellipse(data = subset(boot_vectors, Site %in% c('Salinas')), alpha = 0.5) +
  stat_ellipse(data = subset(boot_vectors, Site %in% c('Charagato')), alpha = 0.5) +
  stat_ellipse(data = subset(boot_vectors, Site %in% c('Dos Mosquises Sur')), alpha = 0.5) +
  stat_ellipse(data = subset(boot_vectors, Site %in% c('Punta Conejo')), alpha = 0.5)
```
