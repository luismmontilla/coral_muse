...
for(i in 1:length(Transectas)) {
  ...
  Factores <- c(Transectas[i], "", strsplit((gsub(".rndpts.csv", "", Transectas[i])), split = "_")[[1]])
  Factores[2:3] <- as.character(Localidades[grep(gsub("data/transects//|data/transects/|.rndpts.csv", "", Transectas[i]), Localidades$File), ])[c(2:3)]
  ...
  ErrorCheck[i, 5]   <- as.character(Localidades[grep(gsub("data/transects//|data/transects/|.rndpts.csv", "", Transectas[i]), Localidades$File), ])[c(5)]
